...
void aleatorio(){
    int TAM=0, op=0, idate=1, iddeco=2;
...
void arquivo(){
    ...
    arquivo.open("src/avioes.txt" , ios::in);
    ...
    if(arquivo.is_open()){
        ...
        while(getline(arquivo,caracteres,'\n')){
            ...
            for(int i=tamanhoAuxiliar; i<=(int)caracteres.size(); i++){
    ...
    if(freeArquivo==1){
        ...
        srand(time(NULL));
        int limiteAterrissagem=0, limiteDecolagem=0, deco=0, aterri=0, opcao=0, soma=0;
